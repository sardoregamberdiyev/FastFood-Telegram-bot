...
add_product = '➕ Yangi menu qo`shish'
delete_category = '🗑️ Kategoriya o`chirish'
...
async def process_settings(message: Message):
    ...
    markup.add(InlineKeyboardButton(
        '+ Yangi mahsulot qo`shish', callback_data='add_category'))

    await message.answer('Kategoriya sozlamalari ⚙️:', reply_markup=markup)
...
async def add_category_callback_handler(query: CallbackQuery):
    await query.message.delete()
    await query.message.answer('Qo`shmoqchi bo`lgan mahsulotingiz nomi ?')
...
async def process_add_product(message: Message):
    ...
    await message.answer('Qo`shomqchi bo`lgan mahsulotingiz nomi ?', reply_markup=markup)
...
async def process_title(message: Message, state: FSMContext):
    ...
    await message.answer('Qo`shmoqchi bo`lgan mahsulotingizni tasvirlab bering 📝', reply_markup=back_markup())
...
async def process_body(message: Message, state: FSMContext):
    ...
    await ProductState.next()
    await message.answer('Mahsulot rasmini yuboring ?', reply_markup=back_markup())
...
async def process_image_photo(message: Message, state: FSMContext):
    ...
    await ProductState.next()
    await message.answer('Narxini kiriting !\n\nNamuna:\n75 qolgan 3 ta 000 ni o`zi qo`shadi (75.000 ming so\'m) '
                         'chiqadi.', reply_markup=back_markup())
...
async def process_price_invalid(message: Message, state: FSMContext):
    ...
    if message.text == back_message:
        ...
        async with state.proxy() as data:
            ...
            await message.answer("Bu rasm mos emas, boshqa rasm kiriting 🔁", reply_markup=back_markup())
...
async def process_confirm_invalid(message: Message, state: FSMContext):
    await message.answer('Bunday imkoniyat yo`q...')
