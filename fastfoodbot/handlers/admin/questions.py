...
async def process_questions(message: Message):
    ...
    if len(questions) == 0:
        ...
        await message.answer('Sizga hozrcha savol va takliflar yo\'q 😊')
        ...
        for cid, question in questions:
            ...
            markup.add(InlineKeyboardButton(
                'Javob berish 🟢', callback_data=question_cb.new(cid=cid, action='answer')))
...
async def process_answer(query: CallbackQuery, callback_data: dict, state: FSMContext):
    ...
    await query.message.answer('Savolga javobni yozing !', reply_markup=ReplyKeyboardRemove())
...
async def process_submit(message: Message, state: FSMContext):
    ...
    await message.answer('Javobingizda xato emaligiga ishonch hosil qiling !', reply_markup=submit_markup())
